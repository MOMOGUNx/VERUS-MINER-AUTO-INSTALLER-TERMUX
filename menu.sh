...
while true; do
    ...
    echo -e "${BLUE}7.${NC} Toggle hybrid mode"
    ...
    case $OPTION in
        ...
        2) echo -ne "${YELLOW}Enter new pool URL: ${NC}"; read POOL; save_config ;;
        3) echo -ne "${YELLOW}Enter algorithm (e.g., verus): ${NC}"; read ALGO; save_config ;;
        ...
        6)
            ...
            # Determine ccminer directory automatically
            ...
            if [[ ! -x ./ccminer ]]; then
                echo -e "${RED}❌ ccminer binary not found or not executable.${NC}"
                sleep 2
                continue
            fi
